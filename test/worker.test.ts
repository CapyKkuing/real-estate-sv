...
describe("routeRequest", () => {
  ...
  it("routes ordinance requests to the public land-use law adapter", async () => {
    const fetchAsset = vi.fn(async () => new Response("asset"))
    const fetchUpstream = vi.fn(async (input: RequestInfo | URL) => {
      const url = new URL(String(input))
      expect(url.pathname).toBe("/1613000/LuLawInfoService/DTluLawInfo")
      return new Response("<response><header><resultCode>00</resultCode></header><body><items><item><UCODE>UQA220</UCODE><UNAME>일반상업지역</UNAME><LAW_CONTENTS>일반상업지역 건폐율 70퍼센트, 용적률 800퍼센트</LAW_CONTENTS></item></items></body></response>")
    })

...
- Modify only when generated by `npm run typecheck`: `worker-configuration.d.ts`
...
```bash
git add src/map-config.ts src/worker.ts site/map-loader.js test/map-config.test.ts test/map-loader.test.js worker-configuration.d.ts
git commit -m "feat: add runtime Kakao map configuration"
```

If `worker-configuration.d.ts` did not change, omit it from `git add`.
